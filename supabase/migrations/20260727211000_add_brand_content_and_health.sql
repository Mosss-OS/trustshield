...
CREATE POLICY "brand_health_owner_all" ON public.brand_health_scores
  FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE INDEX idx_brand_health_user ON public.brand_health_scores(user_id, computed_at DESC);
